...
def ckpt_to_weights_subdir(filepath: Union[str, Path], is_saving) -> Path:
    ...
    if hasattr(filepath, "base_model_path") and filepath.base_model_path.parts[-1] != WEIGHTS_PATH:
        maybe_base_model_path = filepath.base_model_path / WEIGHTS_PATH
        if maybe_base_model_path.is_dir() or is_saving:
            base_dir.base_model_path = maybe_base_model_path
    if is_saving:
